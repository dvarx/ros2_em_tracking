...
Nx=max(sigpowerinterp.grid[0])
Ny=max(sigpowerinterp.grid[1])
Nz=max(sigpowerinterp.grid[2])
last_pos=(Nx/2,Ny/2,Nz/2)
#define localization method
def localize(sigpowers,initial_guess=(Nx/2,Ny/2,Nz/2)):
    residuals=lambda pos : 1e6*(sigpowerinterp(pos)-sigpowers).flatten()
    res=least_squares(residuals,initial_guess,bounds=([0,0,0],[8,8,8]),ftol=1e-5,gtol=1e-5)
    return res
...
yest_filtered=0
zest_filtered=0
#xest_filtered=alpha*xest_filtered+(1-alpha)*x
alpha=0.0
...
def voltage_frames_cb(msg):
    global xest_filtered,yest_filtered,zest_filtered, last_pos, loc_node
    ...
    res=localize(signal_powers,initial_guess=last_pos)
    ...
    if low_pass_pos_filter:
        xest_filtered=alpha*xest_filtered+(1-alpha)*res.x[0]
        yest_filtered=alpha*yest_filtered+(1-alpha)*res.x[1]
        zest_filtered=alpha*zest_filtered+(1-alpha)*res.x[2]
        pointmsgstamped.point.x=(xest_filtered-Nx/2)*grid_spacing_m
        pointmsgstamped.point.y=(yest_filtered-Ny/2)*grid_spacing_m
        pointmsgstamped.point.z=(zest_filtered-Nz/2)*grid_spacing_m
    else:
        pointmsgstamped.point.x=(res.x[0]-Nx/2)*grid_spacing_m
        pointmsgstamped.point.y=(res.x[1]-Ny/2)*grid_spacing_m
        pointmsgstamped.point.z=(res.x[2]-Nz/2)*grid_spacing_m
    ...
    tfmsg.header.frame_id="emns"
    tfmsg.child_frame_id = "pickups"
